...
const WishlistDrawer = ({ show, onClose, wishlistItems = [], onRemove, onMoveToCart }) => {
  console.log("WishlistDrawer rendered with items:", wishlistItems);
  const navigate = useNavigate();

  return (
    <>
      <div className={`cart-backdrop ${show ? "show" : ""}`} onClick={onClose}></div>
          ...
          {wishlistItems.length === 0 ? (
            ...
            wishlistItems.map((item) => (
              <div key={item.id} className="d-flex my-3">
                <img
                  src={item.variant[0]?.images?.[0]?.url || item.images?.[0] || "/fallback.png"}
